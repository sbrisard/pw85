...
Follow this link to :download:`download the above Python script
<./tutorial.py>`.
...
following C program (:download:`download source file <./tutorial.c>`) defines
the two ellipsoids, then computes ``μ²`` (simplified output) and ``λ`` (full
output).

.. literalinclude:: ./tutorial.c
   :language: c

This program is to be compiled as follows::

  gcc tutorial.c -o tutorial -I/path/to/pw85.h -L/path/to/pw85.so -lpw85

on execution, it prints the following lines to ``stdout``
